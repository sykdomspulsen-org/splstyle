...
## Weekly line graph for all geographical areas with vertical x-axis labels

```{r}
# To take a look at how we make line graphs I make new temporary fake data
d_line <- copy(w)
d_line[, n := stats::rpois(.N, 10)]
...
```{r}
q <- ggplot(d_line, aes(x = isoyearweek, y = n, color = location_name, group = location_code))
q <- q + geom_line(lwd = 1)
q <- q + splstyle::theme_fhi_lines_horizontal()
